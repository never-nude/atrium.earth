...
// Laocoön's page uses 0.2 exposure. Native AR must preserve reflectance rather
// than darken the material to match a web camera. Check serialized values and
// retain the separate fix for omitted constant emissive intensity.
...
{
  const converted = makeQuickLookScene(THREE, model, box, { axis: 'y', meters: 2.42 }, { mode: 'surface' });
...
const mapped = prepareQuickLookMaterial(source);
...
console.log('Spatial appearance checks passed: native AR reflectance, constant/mapped emission, source textures, unchanged dimensions and page materials, and per-work WebXR lighting retained.');
