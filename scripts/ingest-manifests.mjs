...
for (const m of additions) {
  ...
  catalog.push({
    ...
    material,
    museum,
    original_location: textValue(m.original_location) || textValue(m.origin),
    current_location: textValue(m.current_location) || textValue(m.location) || museum,
    displayed_at: textValue(m.displayed_at) || museum,
    department: textValue(m.department),
    source_institution: m.source_institution || '',
    source_record_url: textValue(m.source_record_url),
    related_source_record_urls: Array.isArray(m.related_source_record_urls) ? m.related_source_record_urls.map(textValue).filter(Boolean) : [],
    scan_author: textValue(m.scan_author),
