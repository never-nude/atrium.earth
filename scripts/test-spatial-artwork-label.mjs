...
const camera = new URLSearchParams(quickLookLabelFragment(options));
assert.equal(camera.has('custom'), false, 'The default launch must preserve native camera controls');
assert.equal(camera.has('customHeight'), false);
assert.equal(camera.get('allowsContentScaling'), '0');
const params = new URLSearchParams(quickLookLabelFragment({ ...options, showLabel: true }));
