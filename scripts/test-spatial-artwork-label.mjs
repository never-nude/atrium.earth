...
assert.equal(camera.has('custom'), false, 'Native camera controls never use a custom banner');
...
assert.equal(params.has('custom'), false, 'The default labeled scene must preserve the native shutter');
assert.equal(params.has('customHeight'), false);
assert.equal(new URLSearchParams(quickLookLabelFragment({ ...options, showLabel: true, label: long })).has('customHeight'), false);
...
console.log('Artwork labels passed: exactly five catalogue facts, unknown fields, date fallback, authentic material, full-text wrapping, shutter-preserving Apple URL and fixed scale.');
