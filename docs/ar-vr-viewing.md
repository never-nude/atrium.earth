...
The September 15 David reference is the visual target for generated sculpture materials: restrained highlights, readable shadows, and pale warm stone with visible form. `spatial-materials.mjs` prepares those surfaces for Apple AR and WebXR/VR independently of the page's exposure. Atrium marks materials it generated; their palette is applied once instead of multiplying it by the page tint a second time. Material-specific linear luminance bounds keep pale stone light and make dark bronze/wood readable while retaining their hue. These are artistic display defaults, not measured material reflectance. Authored scan textures and unmarked materials retain their existing export treatment.

Native USDZ files explicitly request Apple's modern lighting (`preferredIblVersion = 2`) and retain normal/roughness/metalness shading. Generated stone and bronze use an ordinary `UsdPreviewSurface` with the intended **constant diffuse color**. Their procedural vertex variation is omitted from native export; it remains on the page and in WebXR. This avoids the custom `displayColor` reader that produced gray/purple Venus and gray Thinker in the owner's iPhone captures despite correct-looking Three.js previews. There is no baked page exposure or synthetic emission fill. Apple still controls the environment illumination; a photographic retouch cannot be reproduced exactly under arbitrary room lighting. See [Apple's lighting documentation](https://developer.apple.com/documentation/arkit/specifying-a-lighting-environment-in-ar-quick-look).
...
`scripts/review-spatial-materials.mjs` renders actual archived preview geometry before/after under identical controlled lighting; it is not an Apple renderer emulator. `scripts/audit-ar-appearance.mjs` separately checks all built work-page configurations. The [earlier review record](ar-material-review-2026-09-15/results.json) covers 229 published STL meshes, 49 additional authored-view checks, and three textured scan controls. The controls were pixel-identical under the same test lighting. The [comparison](ar-material-review-2026-09-15/comparison.jpg) shows David, Venus, The Thinker and The Wrestlers. **That review rendered prepared Three.js surfaces, not reimported USDZ files, and missed the native color-transfer failure.** It is retained as a palette review, not evidence of successful Apple rendering.

`test-spatial-appearance.mjs` now serializes and reimports every generated material profile, both with and without source vertex colors, checking the actual bound color and physical material properties. `test-native-material-transfer.mjs` uses the real page, Viewer, and AR launch handler with locally archived models; it saves and checks the exact USDZ offered by the launch link, including original scan textures, labels, and reuse through rotation. It requires the local model archive and does not bypass the site's asset integrity check. The [native export check](ar-native-color-transfer-2026-09-15.json) records Thinker, Venus, David, and Seated Scribe; OpenUSD also opened all four archives and verified the materials bound to their sculpture meshes. Native appearance still needs confirmation on an iPhone.
